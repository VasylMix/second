...
namespace CBAUP {
	CBitArray::CBitArray() {
		pBitAray = std::make_unique<unsigned int[]>(1);
	}
	CBitArray::CBitArray(unsigned int CountBit) : m_nCntBit{ CountBit } {
		pBitAray = std::make_unique<unsigned int[]>((m_nCntBit / SIZE_UI) + 1);
	}
	CBitArray::CBitArray(const CBitArray& other) : CBitArray(other.m_nCntBit) {
		...
		std::memcpy(pBitAray.get(), other.pBitAray.get(), SizeArrayInByte);
	}
	CBitArray::CBitArray(CBitArray&& other) noexcept {
		pBitAray = std::move(other.pBitAray);
